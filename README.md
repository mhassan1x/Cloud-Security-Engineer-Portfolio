# Cloud Security Engineer Portfolio

This repository documents my journey to becoming a Cloud Security Engineer.  
Each day includes hands-on labs, Linux practice, and security configurations on AWS.

## Completed Days
- [Day 1](day1.md) – Cloud Foundations, AWS Account Setup, MFA, IAM Admin, Linux CloudShell.
- [Day 2](day2.md) – Linux permissions (chmod, chown, umask) and IAM deep dive (RBAC vs ABAC).
- [Day 3](day3.md) – Cloud Networking (VPC, Subnets, Security Groups, NACLs, EC2).
- [Day 4](day4.md) – DDoS Protection, AWS WAF, and Cloud Security Standards.
- [Day 5](day5.md) – Data Lifecycle, Encryption (at rest/in transit/in use), AWS KMS (encrypt/decrypt file).
- [Day 6](day6.md) – Container Security (Docker: non‑root, read‑only FS, capability dropping, Trivy scan; Kubernetes: PSS, Network Policies, RBAC, Secrets).
- [Day 7](day7.md) – SecDevOps, CI/CD security, secrets management.
- [Day 8](day8.md) – Disaster Recovery, DR strategies, RTO/RPO.
- [Day 9](day9.md) – Auditing tools (Prowler , ScoutSuite) , incident response basics.
- [Day 10](day10.md) – Cloud Security Automation (Lambda + EventBridge).
- [Day 11](day11.md) – Compliance , Incident Reporting , Audits.
